...
export const useStockData = (enabled: boolean = true) => {
  ...
  // Popular stock symbols (limited to 7 to save API calls)
  ...
  const fetchStockData = async () => {
    ...
    try {
      ...
      if (data?.stocks && data.stocks.length > 0) {
        ...
        console.warn('No stock data returned from API - using demo data');
        ...
        toast({
          ...
          description: "Stock API rate limit reached. Showing realistic sample data.",
        });
      }
    } catch (error: any) {
      ...
      toast({
        title: "Demo Data Active",
        description: "Unable to fetch live data. Using sample stock data.",
      });
    } finally {
      setLoading(false);
    }
